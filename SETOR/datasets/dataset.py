...
def collate_pt(batch, num_dx_classes, num_visit_classes):
    ...
    output = {"input": torch.tensor(inputs).long(),
              ...
              "pid": torch.tensor([x[3] for x in batch])
              }
    return output

def collate_llama2(batch):
...
def load_data(pids, sequences, labels, labels_current_visit, train_ratio=0.8):
    ...
    train_set_pids = [pids[i] for i in train_indices]
    ...
    test_set_pids = [pids[i] for i in test_indices]
    ...
    valid_set_pids = [pids[i] for i in valid_indices]
    ...
    # gli indici vengono ordinati in maniera crescente in base alla lunghezza degli elementi
    # corrispondenti in seq, cioè al numero di visite.
    def len_argsort(seq):
    ...
    train_set = (train_set_x, train_set_y, train_set_y_visit, train_set_pids)
    valid_set = (valid_set_x, valid_set_y, valid_set_y_visit, valid_set_pids)
    test_set = (test_set_x, test_set_y, test_set_y_visit, test_set_pids)

    return train_set, valid_set, test_set


class FTDataset(Dataset):
    def __init__(self, inputs, labels, labels_visit, pids):
        self.inputs = inputs
        self.labels = labels
    ...
    def __getitem__(self, item):
        return self.inputs[item], self.labels[item], self.labels_visit[item], self.pids[item]
