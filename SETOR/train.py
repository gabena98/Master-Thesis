...
def main():
    ...
    """ 
    # select the GPU to use (0-7)
    os.environ["CUDA_VISIBLE_DEVICES"]="2"
    print(os.environ["CUDA_VISIBLE_DEVICES"])
    """

    # select the GPU to use (0-7) using the command line
    device = torch.device("cuda:" + str(args.gpu) if torch.cuda.is_available() else "mps")
    ...
    pids_file = os.path.join(data_path, args.data_source, 'pids.seqs')
    # dictionary files
    ...
    pids = pickle.load(open(pids_file, 'rb'))
    ...
    leaves_list = torch.tensor(np.array(leaves_list)).long().to(device)
    ancestors_list = torch.tensor(np.array(ancestors_list)).long().to(device)
    masks_list = torch.tensor(np.array(masks_list)).float().to(device)
    ...
    torch.save(config, os.path.join(output_dir, 'SETOR_config.pth'))
    ...
    train_set, valid_set, test_set = load_data(pids, inputs, labels, labels_visit, args.train_ratio)
    ...
    #loaded_config = torch.load(os.path.join(output_dir, 'SETOR_config.pth'))
    ...
    for epoch in trange(int(args.num_train_epochs), desc="Epoch"):
        ...
        for phase in ['train', 'val', 'test']:
            ...
            for step, batch in enumerate(tqdm(data_iter, desc="Iteration")):
                ...
                input_ids = batch['input'] # codici ICD-9-CM
                visit_mask = batch['visit_mask'] # maschera per le visite
                code_mask = batch['code_mask'] # maschera per i codici ICD-9-CM
                labels_visit = batch['labels_visit'] # etichette per le visite
                labels_entity = batch['labels_entity'] # etichette per le entità
                ...
                with torch.set_grad_enabled(phase == 'train'):
                    ...
                    if phase == 'train':
                        ...
                        # batch_size(32) x seq_len(e.g.11), num_labels(280)
                        predicts = predicts.reshape(-1, predicts.shape[-1])
                        # batch_size(32) x seq_len(e.g.11), num_labels(280)
                        trues = trues.reshape(-1, trues.shape[-1])
                        
                        # compute precision & accuracy for rank=[5, 10, 15, 20, 25, 30]
    ...
    buf = '{} Best validation accuracy at top 5: {:4f}'.format(time.strftime('%Y-%m-%d %H:%M:%S', time.localtime()),
    ...
    torch.save(model_to_save, output_model_file)
    # Save model weights
    torch.save(model.state_dict(),os.path.join(output_dir, "pytorch_model_weight_only.bin") )
